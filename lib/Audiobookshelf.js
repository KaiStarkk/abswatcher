...
export default class Audiobookshelf {
  ...
  async login() {
    try {
      ...
      } = await axios.post('/login', loginBody, config)
      if (scannerDisableWatcher) {
        logger.error(`Watcher is disabled in Audiobookshelf settings. Please enable it at ${new URL('/config', config.baseURL)} and restart the server`)
        process.exit(1)
      }
  ...
  async getLibraries() {
    return await this.apiCall(async (config) => {
      const {
        data: { libraries }
      } = await axios.get('/api/libraries', config)
      return libraries
    }, true)
  }
    ...
    if (error.response) {
      ...
      switch (error.code) {
        ...
        case 'ERR_CANCELED':
          ...
          break
        case 'EPROTO':
          new URL(this.config.baseURL).protocol === 'https:' ? loggerFunc(`Could not connect to server ${this.config.baseURL}. Change protocol to http?`) : loggerFunc(error)
          break
        default:
          loggerFunc(error)
      }
